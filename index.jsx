...
const FLOW = [
  { id: "welcome", bot: ["Ich bin Alina, Elterngeld-Expertin 👋", "In 60 Sekunden weißt du genau, wie viel dir zusteht."], type: "start" },
  { id: "arbeitsmodell", bot: ["Erste Frage: Arbeitest du angestellt oder machst du dein eigenes Ding?"], type: "select", options: [{ label: "Angestellt", value: "angestellt" }, { label: "Selbständig/Freiberufler", value: "selbstaendig" }] },
  { id: "einkommen_angestellt", bot: ["Wie viel verdienst du ungefähr monatlich (Brutto)?"], type: "select", options: [{ label: "Unter 1.500€", value: 1200 }, { label: "1.500–2.500€", value: 2000 }, { label: "2.500–3.500€", value: 3000 }, { label: "3.500–4.500€", value: 4000 }, { label: "Über 4.500€", value: 5000 }] },
  { id: "einkommen_selbstaendig", bot: ["Wie sah dein durchschnittlicher monatlicher Gewinn aus?"], type: "select", options: [{ label: "Unter 1.500€", value: 1200 }, { label: "1.500–2.500€", value: 2000 }, { label: "2.500–3.500€", value: 3000 }, { label: "3.500–4.500€", value: 4000 }, { label: "Über 4.500€", value: 5000 }] },
  { id: "geschwister", bot: ["Gibt es Geschwister?"], type: "select", options: [{ label: "Ja", value: "ja" }, { label: "Nein", value: "nein" }, { label: "Weiß nicht", value: "nein" }] },
  { id: "phonegate", bot: ["Perfekt! 🎉", "Trag deine Kontaktdaten ein, dann zeige ich dir dein personalisiertes Ergebnis."], type: "phonegate" },
];
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid " + C.green, padding: 24, margin: "8px 0" }}>
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        ...
        <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, fontWeight: 700, color: C.forest, margin: "0 0 6px" }}>Alina kontaktiert dich!</h3>
        <p style={{ fontSize: 13.5, color: C.textMed }}>Trag deine Daten ein.</p>
      </div>
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <button type="submit" disabled={loading} style={{ background: "linear-gradient(135deg," + C.green + "," + C.greenMid + ")", color: "#fff", border: "none", borderRadius: 10, padding: "13px 20px", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          {loading ? "Wird verarbeitet..." : "Weiter zu meinen Zeiten →"}
        </button>
      </form>
    </div>
  );
...
function CallTimeGate({ onSubmit, loading, userName }) {
...
function Result({ result, userName, arbeitsmodell, geschwister }) {
    ...
    <div style={{ background: C.cream, borderRadius: 14, border: "1px solid " + C.border, padding: 20, margin: "8px 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        <div style={{ border: "1px solid " + C.border, borderRadius: 10, padding: 14, background: "#fff" }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: C.accent, marginTop: 6 }}>{result.eg * 12}€ / Jahr</div>
        </div>
        <div style={{ border: "1.5px solid " + C.green, borderRadius: 10, padding: 14, background: C.greenFaint }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: C.green, marginTop: 6 }}>{yearTotal}€ / Jahr</div>
        </div>
      </div>
      ...
      {savings > 0 && (
        <div style={{ background: "#fff3cd", borderLeft: "4px solid #ff9800", padding: 14, marginBottom: 14, fontSize: 14, borderRadius: 8 }}>
          ...
          <div style={{ fontSize: 12, color: "#997700" }}>Das ist der Unterschied zwischen "ich mache es selbst" und "ich lass Alina optimieren".</div>
        </div>
      )}
      ...
      <div style={{ background: "#f5f5f5", borderRadius: 8, padding: 14, marginBottom: 14, fontSize: 13.5, color: C.text, lineHeight: 1.6 }}>
        <div style={{ fontWeight: 700, marginBottom: 8 }}>⚠️ Aber ehrlich gesagt:</div>
        <p style={{ margin: "0 0 8px 0" }}>Den Antrag selbst zu optimieren ist aufwendig. Es geht um Steuerklasse, Gewinnermittlung, Besonderheiten... eine falsche Angabe kostet dich direkt € aus der Tasche.</p>
        <p style={{ margin: 0 }}>Mit Alina zahlst du {price}€ einmalig — und hast die Sicherheit, dass ALLES korrekt ist. Dafür brauchst du nur 2 Stunden deiner Zeit.</p>
      </div>
    </div>
  );
}
        ...
        <section style={{ maxWidth: 720, margin: "0 auto", padding: "0 20px 24px" }}>
          <div style={{ background: "#fff", borderRadius: 18, border: "1px solid " + C.border, overflow: "hidden" }}>
            <div ref={chatRef} style={{ padding: "20px 18px", overflowY: "auto", maxHeight: "58vh", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {showRes && result && <Result result={result} answers={answers} userName={uName} arbeitsmodell={answers.arbeitsmodell} geschwister={answers.geschwister} />}
              {showCallTime && !completed && <CallTimeGate onSubmit={onCallTime} loading={submitting} userName={uName} />}
            </div>
